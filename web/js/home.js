...
let signupFormT = document.getElementById("Formsignup");
...
//     console.log(
//       "Postal code does not match the format for the selected country."
//     );
//   } else {
//     console.log("The postal code follows the format for the selected country.");
//   }
// });
postalCode.addEventListener("input", function () {
...
function validateEmail() {
  ...
  emailInput.addEventListener("input", function () {
    if (!email.match(regexEmail)) {
      emailInput.value = emailInput.value.replace(/[^a-zA-Z0-9@.\-]/g, ""); //[^a-zA-Z]
    }
  });
...
password.addEventListener("input", function () {
  ...
  let sth = strength(inputValue);

  //if (inputValue.value === "none") { }
  if (sth <= 0) {
...
//   let pattern = /^[A-Za-z0-9#.?!@$%^&*-]{8,72}$/;
//   if (!this.value.match(pattern)) {
//     this.value = this.value.replace(/[^\w#.?!@$%^&*-]/g, ""); //[^a-zA-Z]
//   }
// });
password.addEventListener("input", (event) => {
  let passvalue = event.target.value;

  // passvalue = passvalue.replace(/[^a-zA-Z0-9#.?!@$%^&*-]/g, "");

  let pattern = /^[A-Za-z0-9#.?!@$%^&*-]{8,72}$/;
  if (!passvalue.match(pattern)) {
    passvalue = passvalue.replace(/[^\w#.?!@$%^&*-]/g, ""); //[^a-zA-Z]
  }

  updateRequirement("length", passvalue.length >= 8 && passvalue.length <= 72);
  updateRequirement("lowercase", /[a-z]/.test(passvalue));
  updateRequirement("uppercase", /[A-Z]/.test(passvalue));
  updateRequirement("number", /\d/.test(passvalue));
  updateRequirement("characters", /[#.?!@$%^&*-]/.test(passvalue));

  event.target.value = passvalue;
});

signupFormT.addEventListener("submit", (e) => {
  let passValue = signupFormT.querySelector('input[type="password"]').value;
  ...
  const isValidPassword =
    lowerCaseRegex.test(passValue) &&
    upperCaseRegex.test(passValue) &&
    digitRegex.test(passValue) &&
    specialCharRegex.test(passValue) &&
    lengthRegex.test(passValue);
